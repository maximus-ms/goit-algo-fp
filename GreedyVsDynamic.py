...
def greedy_algorithm(budget):
    ...
    res = []
    effective_q = [
        (val["cost"] / val["calories"], name) for name, val in items.items()
    ]
...
def dynamic_programming(budget):
    ...
    if K is None:
        ...
        for i in range(1, n + 1):
            p_i = i - 1
            for cost_i in range(1, total_cost + 1):
                res = K_[p_i][cost_i]
                prev_cost = values[p_i]["cost"]
                if prev_cost <= cost_i:
                    v = (
                        K_[p_i][cost_i - prev_cost][0]
                        + values[p_i]["calories"]
                    )
                    if res[0] < v:
                        res_names = list(K_[p_i][cost_i - prev_cost][1])
